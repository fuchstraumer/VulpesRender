...
namespace vpr
{
    ...
    GraphicsPipeline::GraphicsPipeline(const VkDevice& _parent, VkGraphicsPipelineCreateInfo info, VkPipeline _handle) : parent(_parent), createInfo(std::move(info)), handle(_handle) {}

    GraphicsPipeline::GraphicsPipeline(const VkDevice& _parent) : parent(_parent), createInfo(vk_graphics_pipeline_create_info_base), handle(VK_NULL_HANDLE) {}
    ...
    {
        VkResult result = vkCreateGraphicsPipelines(parent, cache, 1, &create_info, nullptr, &handle);
        VkAssert(result);
    ...
    void GraphicsPipeline::Destroy()
    {
        ...
        {
            vkDestroyPipeline(parent, handle, allocators);
        }
    }
    ...
    void GraphicsPipeline::CreateMultiple(const VkDevice& dvc, const VkGraphicsPipelineCreateInfo* infos, const size_t num_infos, VkPipelineCache cache, GraphicsPipeline** results)
    {
        std::vector<VkPipeline> handles(num_infos);
        VkResult result = vkCreateGraphicsPipelines(dvc, cache, static_cast<uint32_t>(num_infos), infos, nullptr, handles.data());
        VkAssert(result);
