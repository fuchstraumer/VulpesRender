...
namespace vpr {
    ...
    const static VkBufferViewCreateInfo base_view_info{ VK_STRUCTURE_TYPE_BUFFER_VIEW_CREATE_INFO, nullptr, 0, VK_NULL_HANDLE, VK_FORMAT_UNDEFINED, 0, 0 };

    Buffer::Buffer(const VkDevice& _parent) : parent(_parent), createInfo(vk_buffer_create_info_base), viewCreateInfo(base_view_info),
    ...
    Buffer::Buffer(Buffer && other) noexcept : size(std::move(other.size)), createInfo(std::move(other.createInfo)), 
